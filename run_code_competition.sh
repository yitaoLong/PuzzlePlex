...
single_player_stochastic_puzzles=('RubyRisksPuzzle' 'ExclusivityProbesPuzzle' 'MaxTargetPuzzle')
for model in "${model_list[@]}"; do
    for puzzle in "${single_player_deterministic_puzzles[@]}"; do
        for difficulty in "${difficulty_levels[@]}"; do
            python3 run_code_competition.py \
                --input_dir 'result_code' \
                --puzzle_name "$puzzle" \
                --model_1 "$model" \
                --num_players 1 \
                --difficulty "$difficulty" \
                --seed_range '1-10' \
                --output_dir 'code_competition_results'
        done
    done
    for puzzle in "${single_player_stochastic_puzzles[@]}"; do
        for difficulty in "${difficulty_levels[@]}"; do
            python3 run_code_competition.py \
                --input_dir 'result_code' \
                --puzzle_name "$puzzle" \
...
two_player_stochastic_puzzles=('BeatOrBombStoPuzzle' 'LargerTargetPuzzle')
for ((idx1=0; idx1<${#model_list[@]}; idx1++)); do
    for ((idx2=idx1+1; idx2<${#model_list[@]}; idx2++)); do
        ...
        for puzzle in "${two_player_stochastic_puzzles[@]}"; do
            for difficulty in "${difficulty_levels[@]}"; do
                python3 run_code_competition.py \
                    --input_dir 'result_code' \
                    --puzzle_name "$puzzle" \
