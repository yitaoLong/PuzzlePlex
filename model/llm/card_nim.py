        ...
        for _ in range(5):
            if message.generated_type == 'text':
                try:
                    ...
                    # extract the single digit after 'Operation:'
                    extracted = re.search(r'Operation:\s*(?:`?List\[int\]`?:\s*\[(\d+)\]|\[(\d+)\])', response) or re.search(r'Operation:\s*(?:\`?\[(\d+)\]\`?|`\[(\d+)\]`)', response) or re.search(r"'content':\s*'?\[(\d+)\]'?", response)
                    if extracted:
                        extracted = extracted.group(1) or extracted.group(2)
                        extracted = '[' + extracted + ']'
