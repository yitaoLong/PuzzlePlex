...
class SuperplyMLLMStrategy:
    ...
    def receive_message(self, message: Info) -> Info:
        for _ in range(5):
            try:
                response, self.tmp_history, self.tmp_reasoning_content, self.tmp_completion_token_count, self.tmp_reasoning_token_count, self.tmp_total_token_count = self._model.generate(message.message)
                self.cnt_raw_output = response
                # extract the (row_index, col_index) from the response
                extracted = re.findall(r'\((\d+),(\d+)\)', response.replace('\n', '').replace(' ', ''))
                row_index, col_index = int(extracted[-1][0]), int(extracted[-1][1])
                return_value = (row_index, col_index)
