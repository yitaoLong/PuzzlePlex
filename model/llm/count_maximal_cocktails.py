...
class CountMaximalCocktailsLLMStrategy:
            ...
            if message.generated_type == 'text':
                try:
                    ...
                    if message.difficulty == 'easy':
                        # extract the int value from the response
                        extracted = re.findall(r'\d+', response)
                        return_value = int(extracted[-1])
                        ...
                        # extract the List[List[int]] from the response
