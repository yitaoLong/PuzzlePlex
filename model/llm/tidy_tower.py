    ...
    def receive_message(self, message: Info) -> Info:
        if len(message.message) > 1 and message.generated_type == 'text':
            ...
            for i in range(5):
                for _ in range(5):
                    try:
                        ...
                        pattern = r"operation\s*=\s*\[(\d+,\s*\d+,\s*\d+)\]"
                        extracted = re.findall(pattern, response.lower())
                        extracted = '[' + extracted[-1] + ']'
                        extracted = eval(extracted)
                        if len(extracted) > 3:
                            extracted = extracted[:2]

                        return_value_list.append(extracted)
            ...
            for i in range(5):
                ...
                model_input += '''Please choose the best choice and output in the format: 'operation = [Cube index, Rotation, Holding]'.'''
                for _ in range(5):
                    try:
                        response, self.tmp_history, self.tmp_reasoning_content, self.tmp_completion_token_count, self.tmp_reasoning_token_count, self.tmp_total_token_count = self._model.generate(message.message[0].data, message.with_history)
                        self.cnt_raw_output = response
                        ...
                        for j in range(len(return_value_list)):
                            if return_value_list[j] == extracted:
                                index = j
                                break
                        if index != -1:
            ...
            for _ in range(5):
                if message.generated_type == 'text':
                    try:
                        ...
                        # check if not iterative
                        extracted = re.findall(r'\[\[(.*?)\]\]', response.replace('\n', '').replace(' ', ''))
