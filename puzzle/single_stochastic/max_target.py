...
from prompts.max_target import *
...
class MaxTargetPuzzle(BasePuzzle):
    bag_count: int = 0
    ...
    def puzzle_generator(self, models: List[BaseStrategy], difficulty: str, one_shot: bool, tot: bool, iterative: bool, simplified_description: bool, legal_candidates: bool, with_history: bool, code_generation: bool, random_seed: Any, output_dir: str) -> List[Info]:
        ...
        self.guess_bag = []
        self.cnt_guess = 0
        self.value_received = 0
        self.value = 0
        
        self.scores = ['0']

        if self.difficulty == 'easy':
            self.bag_count = random.randint(3, 7)
        elif self.difficulty == 'normal':
            self.bag_count = random.randint(8, 12)
        ...
        for bag in self.true_bag:
            count += len(bag)
        self.max_guess = int(count * random.uniform(0.5, 0.7))
        ...
        init_dict = {'true_bag': self.true_bag, 'random_bag': self.random_bag, 'max_guess': self.max_guess, 'guess_bag': self.guess_bag, 'value_received': self.value_received, 'empty_bag': []}
        ...
        if self.code_generation:
            ...
            if model.strategy_type.value != 'LLM':
                ...
                nl_description = DESCRIPTION.format(bag_count=self.bag_count, bag_coins=bag_coins, max_guess=self.max_guess)
    ...
    def state_transition_checker(self, message: Info, response: Info, model: BaseStrategy):
        ...
        if result is None:
            ...
            elif len(self.tracking_bag[result]) == 0:
                return message, StateLegality.TERMINATE
            else:
    ...
    def state_transition(self, response: Info, model: BaseStrategy, next_model: Any):
        ...
        self.value_received += self.value
        self.scores = [str(self.value_received)]

        null_bag = []
        for i in range(len(self.tracking_bag)):
            if len(self.tracking_bag[i]) == 0:
                null_bag.append(i)
        tmp = {'true_bag': self.true_bag, 'random_bag': self.random_bag, 'max_guess': self.max_guess, 'guess_bag': self.guess_bag, 'value_received': self.value_received, 'empty_bag': null_bag}
        
        self.history['state'].append(f'guess_bag: {self.guess_bag[-1][0]}, value: {self.guess_bag[-1][1]}, value_received: {self.value_received}, empty_bag: {null_bag}')
        ...
        if model.strategy_type.value != 'LLM':
            ...
            nl_description = ''
            nl_description += f'''Your last guess was {self.cnt_guess} and you received a coin value of {self.value}. The total value you have received so far is {self.value_received}.'''
            if len(null_bag) > 0:
            ...
            nl_description += f'''\n\nYou have {self.max_guess - self.guess_count} picks left. Please make your next pick.'''
    ...
    def calculate_score(self, game_status: GameStatus, current_player: int):
        if game_status == GameStatus.END:
            self.scores = [str(self.value_received)]
            return self.value_received
        else:
            self.scores = ['Invalid - 0']
            return 0
