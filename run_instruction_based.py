...
if __name__ == '__main__':
    ...
    if args.num_players == 1:
        ...
        model1 = ModelBase(name=args.model_1, max_new_tokens=args.max_new_tokens)
        model2 = ModelBase(name=args.model_2, max_new_tokens=args.max_new_tokens)
        model1.load_model()
        model2.load_model()
    ...
    for seed in seeds:
        ...
        system = BaseSystem()
        if args.num_players == 1:
            if 'custom' in args.model_1:
                model_instance1 = puzzle_custom_list[index](model1)
            else:
                model_instance1 = puzzle_llm_list[index](model1)
            score = system.run(web=args.web, output_dir = args.output_dir, num_players=args.num_players, model_name_list=[model_instance1], puzzle_name=puzzle, max_time_cost=args.max_time_cost, difficulty=args.difficulty, one_shot=args.one_shot, tot=args.tot, iterative=args.iterative, simplified_description=args.simplified_description, legal_candidates=args.legal_candidates, with_history=args.with_history, code_generation=args.code_generation, random_seed=seed)
            print('Score:', score)
            model1.clear()
        else:
            if 'custom' in args.model_1:
                ...
                model_instance1 = puzzle_llm_list[index](model1)
            if 'custom' in args.model_2:
                model_instance2 = puzzle_custom_list[index](model2)
            else:
                model_instance2 = puzzle_llm_list[index](model2)
            score = system.run(web=args.web, output_dir = args.output_dir, num_players=args.num_players, model_name_list=[model_instance1, model_instance2], puzzle_name=puzzle, max_time_cost=args.max_time_cost, difficulty=args.difficulty, one_shot=args.one_shot, tot=args.tot, iterative=args.iterative, simplified_description=args.simplified_description, legal_candidates=args.legal_candidates, with_history=args.with_history, code_generation=args.code_generation, random_seed=seed)
            print('Score:', score)
            model1.clear()
            model2.clear()